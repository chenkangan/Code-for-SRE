...
for (i in c(16,5)) {
  ...
  i00=i
  print(i)
  #####n will change to 1:length(data1) after parametric
  n=length(data[[i]])
  data0=c(0)
  data1=c(0)
  p=1
  ...
  c=ndat/25
  ###data
  plot(data1,0:(length(data1)-1),type='l',lwd=4,xlab = 'time',ylab='Number of failures',main=paste('Behavior of mean value function on data',data_name[i]))
...
# saveRDS(results_list, file.path("analysis_result", "DCAS", "DCAS_results_1.rds"))
